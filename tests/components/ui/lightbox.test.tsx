...
describe("Lightbox", () => {
  ...
  it("renders a single image in legacy src mode", () => {
    render(<Lightbox src="https://x/p.webp" onClose={() => {}} />);
  ...
  it("renders the initialIndex in gallery mode and shows counter", () => {
    render(
      <Lightbox
        images={[{ src: "u1" }, { src: "u2", caption: "second" }, { src: "u3" }]}
        initialIndex={1}
        onClose={() => {}}
      />
    );
  ...
  it("places captions on a resolved semantic inverse surface", () => {
    render(<Lightbox images={[{ src: "u1", caption: "Newsroom caption" }]} onClose={() => {}} />);

    expect(screen.getByText("Newsroom caption")).toHaveClass("bg-inverse", "text-text-inverse");
  });

  it("advances with ArrowRight and wraps around", () => {
    render(<Lightbox images={[{ src: "u1" }, { src: "u2" }]} onClose={() => {}} />);
  ...
  it("calls onClose on Escape", () => {
    const close = vi.fn();
    render(<Lightbox images={[{ src: "u1" }]} onClose={close} />);
  ...
  it("renders as a labelled modal and focuses its Close control", async () => {
    ...
    expect(screen.getByRole("dialog", { name: "Photo viewer" })).toHaveAttribute(
      "aria-modal",
      "true"
    );
    await waitFor(() =>
