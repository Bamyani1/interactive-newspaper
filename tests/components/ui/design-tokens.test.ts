...
describe("canonical design tokens", () => {
  ...
  it("pairs every type size with its documented line height", () => {
    ...
    for (const [name, value] of Object.entries(expected)) {
      ...
      expect(theme).toContain(`--text-${name}--line-height: var(--leading-${name});`);
    }
    expect(declarations(type, "--tracking-tighter")[0]).toBe("var(--tracking-tight)");
  });
  ...
  it("keeps one primitive geometry contract and a wrapping mobile gallery", () => {
    ...
    expect(gallery).toContain('data-testid="button-size-row" className="flex max-w-full flex-wrap');
  });
