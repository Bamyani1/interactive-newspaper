...
import { assertHoldoutScoringAllowed } from "../../scripts/rag/verify-evaluation-freeze";
import { scoreRun, lockAcceptanceBands, type EvidenceCatalog } from "../../scripts/rag/score-eval";
import { loadBlindQuestions, orderQuestions } from "../../scripts/rag/run-eval";
import {
...
function makeRunFile(overrides: { runId: string; datasetId: string; split: string }): EvalRunFile {
  const records = [
...
describe("holdout scoring gate", () => {
  ...
  describe("scoreRun in-process guard", () => {
    it("refuses a holdout run file without bands", () => {
      expect(() => scoreRun({ runFile: holdoutRun, evidence: holdoutEvidence })).toThrow(
        /locked acceptance bands/
      );
    });
    ...
    it("never gates development run files", () => {
      ...
      expect(() => scoreRun({ runFile: devRun, evidence: devEvidence })).not.toThrow();
    });
  });
});
...
describe("runner blindness", () => {
  ...
  it("static guard (lexical only — cannot catch dynamic key access): run-eval.ts source names no holdout/dev evidence field", () => {
    const source = readFileSync(path.resolve(__dirname, "../../scripts/rag/run-eval.ts"), "utf8");
