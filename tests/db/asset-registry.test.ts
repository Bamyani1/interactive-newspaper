...
describe("parseImageReference", () => {
  ...
  it("parses IMAGE_BASE_URL-prefixed legacy URLs, including extra path prefixes", () => {
    const parsed = parseImageReference("https://images.example.com/1955-03-09/images/photo_1.webp");
...
describe("listR2Objects", () => {
  it("paginates fully across both prefixes and dedupes overlapping keys", async () => {
    ...
    expect(
      objects.filter((object: { key: string }) => object.key === `ocr-assets/${H1}.webp`)
    ).toHaveLength(1);
...
describe("writeRegistryArtifact", () => {
  it("writes immutable artifacts and refuses divergent overwrites", () => {
    ...
    expect(path.basename(first.jsonPath)).toBe(`registry-${registry.sha256!.slice(0, 16)}.json`);
...
describe("collectDbReferences + applyRegistry (PGlite)", () => {
  ...
  beforeAll(async () => {
    ...
    await db.executor.query({
      ...
      params: [JSON.stringify(["/api/editions/1955-03-09/images/photo_1.jpg", "mystery-blob"])],
    });
    await db.executor.query({
