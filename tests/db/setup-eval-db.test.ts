...
describe("setup-eval-db (PGlite)", () => {
  ...
  it("throws on a count mismatch, listing frozen-corpus and database counts", async () => {
    ...
    try {
      ...
      ).rejects.toThrow(/editions: frozen corpus declares 5, evaluation database has 2/);
    } finally {
      await evalDb.close();
    }
  });
  ...
  it("throws on schema divergence, naming the differing schema object", async () => {
    ...
    try {
      ...
      ).rejects.toThrow(`indexes.${droppedIndex}: present in snapshot, missing from live schema`);
  ...
  describe("assertNotProductionUrl", () => {
    ...
    it("throws when eval URL shares host AND database with the production URL", () => {
      expect(() =>
        assertNotProductionUrl("postgres://other:pw2@db.prod.example.com/newspaper", PROD, {
          env: {},
        })
    ...
    it("passes when the database name differs and contains 'eval'", () => {
      expect(
        assertNotProductionUrl("postgres://app:pw@db.prod.example.com/newspaper_eval", PROD, {
          env: {},
        })
    ...
    it("throws when prod URL is unset but eval URL equals a DATABASE_URL in env", () => {
      ...
      expect(() => assertNotProductionUrl(url, undefined, { env: { DATABASE_URL: url } })).toThrow(
        /equals DATABASE_URL in the environment/
      );
