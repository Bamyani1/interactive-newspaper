...
describe("identity backfill and compat against PGlite", () => {
  ...
  it("rejects UPDATEs on content_revisions (immutability trigger)", async () => {
    const { rows } = await db.pg.query<{ id: string }>("SELECT id FROM content_revisions LIMIT 1");
    expect(rows).toHaveLength(1);
    await expect(
      db.pg.query("UPDATE content_revisions SET headline = 'tampered' WHERE id = $1", [rows[0].id])
