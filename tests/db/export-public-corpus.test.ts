...
describe("export-public-corpus (PGlite)", () => {
  ...
  it("manifest lists exactly the allowlisted tables with correct counts and hashes", () => {
    ...
    expect(Object.fromEntries(manifest.tables.map((t) => [t.name, t.rowCount]))).toEqual({
      editions: 2,
      articles: 2,
      ads: 1,
      weather: 1,
    ...
    for (const table of manifest.tables) {
      expect(sha256OfFile(path.join(outDir, `${table.name}.jsonl`)), table.name).toBe(table.sha256);
    }
    ...
    expect(manifest.excludedColumns).toEqual(EXCLUDED_COLUMNS.map((entry) => ({ ...entry })));
