...
import { buildRegistry, parseImageReference } from "../../scripts/db/bootstrap-asset-registry.mjs";
import {
  GC_STATE_KEY,
...
describe("protection", () => {
  it("marks only unreferenced editions when the registry knows a partial world", async () => {
    ...
    const registry = makeRegistry(
      ["1955-03-09/images/photo_1.webp", "1955-03-16/images/photo_1.webp", `ocr-assets/${H1}.webp`],
      [
        { key: "1955-03-09/images/photo_1.webp", size: 10 },
...
describe("apply gating and grace-ledger CAS", () => {
  ...
  it("refuses --apply without a matching approval token, even in tests", async () => {
    ...
    const client = createFakeClient({ objects });
    await expect(runGc({ registry, client, apply: true, approvalToken: null })).rejects.toThrow(
      /approval-token/
    );
    expect(client.calls.list).toHaveLength(0);
    expect(client.calls.deletes).toHaveLength(0);
    ...
    await expect(runGc({ registry, client, apply: true, approvalToken: "wrong" })).rejects.toThrow(
      /approval-token/
    );
    expect(client.calls.deletes).toHaveLength(0);
  });
