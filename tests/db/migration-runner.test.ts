...
describe("runMigrations against a real database", () => {
  ...
  it(
    "fresh apply records every on-disk migration id and checksum in the ledger",
    async () => {
  ...
  it(
    "second run is a no-op: everything alreadyApplied, nothing applied or skipped",
    async () => {
      const result = await runMigrations(db.executor);
  ...
  it(
    "releases the advisory lock after a successful run",
    async () => {
      const rows = await db.executor.query({
...
describe("applied-migration immutability guards", () => {
  ...
  it(
    "throws naming the migration when an applied file's checksum changes",
    async () => {
  ...
  it(
    "throws naming the migration when an applied file is deleted",
    async () => {
      const dir = makeTempDir();
      tempDirs.push(dir);
      cpSync(MIGRATIONS_DIR, dir, { recursive: true });
...
describe("ledger-race atomicity", () => {
  it(
    "rolls back the whole batch (DDL included) when the ledger insert conflicts",
    async () => {
      const dir = makeTempDir();
      const db = await createTestDb();
      try {
  ...
  it("reports the lost race as skipped instead of throwing", async () => {
    const stub: QueryExecutor = {
      ...
      async transactionBatch() {
        throw Object.assign(
          new Error("duplicate key value violates unique constraint schema_migrations_pkey"),
          {
            code: "23505",
          }
        );
