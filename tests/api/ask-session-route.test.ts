...
describe("GET /api/ask/session", () => {
  beforeEach(() => {
    ...
    (sessionHasAnyTurns as ReturnType<typeof vi.fn>).mockResolvedValue(false);
    (fetchArticlesByIds as ReturnType<typeof vi.fn>).mockResolvedValue(new Map());
  });
  ...
  it("hydrates each turn with its full answer and sourceArticles fetched from the articles table", async () => {
    ...
    const callArg = (fetchArticlesByIds as ReturnType<typeof vi.fn>).mock.calls[0][0] as string[];
...
describe("DELETE /api/ask/session", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (deleteConversationTurns as ReturnType<typeof vi.fn>).mockResolvedValue({ ok: true });
  });
  ...
  it("returns 500 with an error body when the store reports a failed delete", async () => {
    (deleteConversationTurns as ReturnType<typeof vi.fn>).mockResolvedValue({
      ok: false,
      error: "neon down",
    });
