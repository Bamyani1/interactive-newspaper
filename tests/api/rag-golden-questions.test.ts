...
const developmentCatalog: DevelopmentCatalog = JSON.parse(readFileSync(catalogPath, "utf-8"));
...
describe.skipIf(!process.env.RUN_RAG_GOLDEN)("RAG golden regression suite", () => {
  ...
  beforeAll(async () => {
    ...
    if (!process.env.DATABASE_URL) {
      throw new Error("Golden suite requires DATABASE_URL (via .env.local or shell env)");
    }
    if (!process.env.GOOGLE_CLOUD_PROJECT) {
      throw new Error("Golden suite requires GOOGLE_CLOUD_PROJECT and working ADC");
    }
  ...
  afterAll(() => {
    ...
    if (existsSync(baselinePath)) {
      try {
        const baselineRaw = JSON.parse(readFileSync(baselinePath, "utf-8")) as {
          capturedAt?: string;
        ...
        for (const curr of observed) {
          ...
          if (curr.confidence && prior.confidence) {
            const drop = CONFIDENCE_RANK[prior.confidence] - CONFIDENCE_RANK[curr.confidence];
            if (drop >= 2) {
              console.error(
                `[rag-golden] INFO  ${curr.id}: confidence ${prior.confidence} → ${curr.confidence}`
              );
            } else if (drop === 1) {
              console.error(
          ...
          if (prior.citations >= 2) {
            if (curr.citations * 2 < prior.citations) {
              console.error(
                `[rag-golden] INFO  ${curr.id}: citations ${prior.citations} → ${curr.citations}`
              );
          ...
          if (curr.method && prior.method && curr.method !== prior.method) {
            console.error(`[rag-golden] WARN  ${curr.id}: method ${prior.method} → ${curr.method}`);
          }
        }
      } catch (err) {
        console.error(`[rag-golden] failed to read baseline for comparison: ${err}`);
      }
    }
  });
    ...
    it(
      ...
      async () => {
        ...
        if (q.keywordsAny && q.keywordsAny.length > 0) {
          ...
          const found = q.keywordsAny.some((kw) => answerLower.includes(kw.toLowerCase()));
          expect(
            found,
