...
const validBody = {
  ...
  mode: "text" as const,
  citations: [{ articleId: "1965-01-07-0", headline: "Test", editionDate: "1965-01-07" }],
};
...
describe("POST /api/ask/feedback", () => {
  ...
  it("rejects an invalid JSON body with 400", async () => {
    const response = await POST(makeRequest("not json {") as unknown as Parameters<typeof POST>[0]);
  ...
  it("rejects missing requestId with 400", async () => {
    ...
    void requestId;
    const response = await POST(makeRequest(rest) as unknown as Parameters<typeof POST>[0]);
    expect(response.status).toBe(400);
    const body = await response.json();
  ...
  it("sanitizes an empty/whitespace comment to null", async () => {
    ...
    await POST(
      makeRequest({ ...validBody, vote: "down", comment: "   " }) as unknown as Parameters<
        typeof POST
      >[0]
    );
  ...
  it("drops unknown confidence values", async () => {
    await POST(
      makeRequest({ ...validBody, confidence: "super-high" }) as unknown as Parameters<
        typeof POST
      >[0]
    );
    const [, ...values] = mockSql.mock.calls[0];
  ...
  it.each([
    ...
    async (_label, contentRevisionId) => {
      ...
      const response = await POST(
        makeRequest({ ...validBody, citations: [citation] }) as unknown as Parameters<
          typeof POST
        >[0]
      );
