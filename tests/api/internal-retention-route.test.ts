...
describe("/api/internal/retention", () => {
  ...
  it("returns 401 when the Authorization header is missing", async () => {
    const response = await route.POST(makeRequest("POST") as unknown as RouteRequest);
    expect(response.status).toBe(401);
    await expect(response.json()).resolves.toEqual({ error: "Unauthorized" });
    expect(mockRunRetentionSweep).not.toHaveBeenCalled();
  });
  ...
  it("returns 200 with the sweep counts on POST with the correct bearer", async () => {
    ...
    expect(mockCreateNeonExecutor).toHaveBeenCalledWith("postgres://mocked/never-connects");
