...
test("primitive gallery fits a 390px reduced-motion viewport", async ({ page }) => {
  ...
  expect(await sizeRow.evaluate((element) => getComputedStyle(element).flexWrap)).toBe("wrap");
  ...
  const iconBox = await page.getByRole("button", { name: "Open controls" }).boundingBox();
