...
test("TimeControls lets Tab move focus before closing its date popup", async ({ page }) => {
  ...
  await expect(page.getByRole("textbox", { name: "Search the archive" })).toBeFocused();
  await expect(listbox).toBeHidden();
});

test("Mobile More lets Shift+Tab reach its trigger before closing", async ({ page, isMobile }) => {
