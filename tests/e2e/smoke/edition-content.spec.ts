...
async function expectNoHorizontalOverflow(page: Page) {
  ...
  const feedFits = await page
    .locator(".edition-feed-surface")
...
test.describe("representative edition content", () => {
  test("loads context only at desktop width", async ({ page, diagnostics }) => {
    ...
    if ((page.viewportSize()?.width ?? 0) >= 1024) {
      ...
      await expect.poll(() => contextRequests).toContain("/top-10-music/chart-1950-2010.json");
    } else {
  ...
  test("native Enter opens a representative news section", async ({ page, diagnostics }) => {
    ...
    if ((page.viewportSize()?.width ?? 0) < 640) {
      await page.getByRole("button", { name: "More sections" }).click();
      await expect(page.getByRole("menuitem", { name: "Sports", exact: true })).toHaveAttribute(
        "aria-current",
        "page"
      );
    } else {
