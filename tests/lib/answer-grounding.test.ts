...
import { groundAgentAnswer, groundPipelineAnswer } from "@/src/lib/answer-grounding";
...
describe("answer grounding", () => {
  it("removes invented source markers, links, and image URLs", () => {
    ...
    expect(result.citations.map((citation) => citation.articleId)).toEqual(["1960-01-07-0"]);
  ...
  it("enforces the three-image output cap", () => {
    ...
    const result = groundPipelineAnswer(
      "[Source 1] ![x](https://a/1) ![x](https://a/2) " + "![x](https://a/3) ![x](https://a/4)",
      [source]
    );
