...
import { getCachedAnswer, setCachedAnswer, clearAnswerCache } from "@/src/lib/answer-cache";
...
describe("answer-cache", () => {
  ...
  it("bypasses reads and writes during an isolated evaluation", async () => {
    ...
    setCachedAnswer("evaluation-only question", {}, makeResponse({ answer: "must not persist" }));
  ...
  it("different filters produce different keys", async () => {
    ...
    expect((await getCachedAnswer("same question", { category: "News" }))!.answer).toBe(
      "with filter"
    );
  });
