...
describe("RAG evaluation configuration", () => {
  ...
  it("requires a run and frozen corpus identity", () => {
    expect(() =>
      getRagEvaluationConfig({ RAG_EVALUATION_MODE: "1" } as unknown as NodeJS.ProcessEnv)
    ).toThrow(/RAG_EVALUATION_RUN_ID/);
    expect(() =>
      getRagEvaluationConfig({
  ...
  it("rejects ambiguous flag values", () => {
    expect(() =>
      isRagEvaluationMode({ RAG_EVALUATION_MODE: "yes" } as unknown as NodeJS.ProcessEnv)
    ).toThrow(/Invalid RAG_EVALUATION_MODE/);
  });
});
