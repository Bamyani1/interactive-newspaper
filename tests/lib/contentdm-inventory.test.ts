...
describe("CONTENTdm source inventory", () => {
  ...
  it("flags same-date and duplicate-title collisions without choosing a winner", () => {
    ...
    expect(records.every((record) => record.reviewFlags.includes("same_date_collision"))).toBe(
      true
    );
    expect(
      records.every((record) => record.reviewFlags.includes("duplicate_title_collision"))
    ).toBe(true);
  });
