...
describe("SourceReader — accessible modal behavior", () => {
  ...
  it("labels the dialog, focuses Close, locks scroll, and makes the page inert", async () => {
    const { container } = render(<SourceReader source={makeSource()} onClose={vi.fn()} />);

    expect(await screen.findByRole("dialog", { name: "Test Article" })).toHaveAttribute(
      "aria-modal",
      "true"
    );
    expect(screen.getByRole("button", { name: /close article reader/i })).toHaveFocus();
  ...
  it("lets Escape close a nested photo viewer before the source reader", async () => {
    ...
    fireEvent.keyDown(document, { key: "Escape" });
    await waitFor(() => expect(screen.queryByRole("dialog", { name: "Photo viewer" })).toBeNull());
    expect(reader).toBeInTheDocument();
    expect(photo).toHaveFocus();

    fireEvent.keyDown(document, { key: "Escape" });
    await waitFor(() => expect(screen.queryByRole("dialog", { name: "Test Article" })).toBeNull());
    expect(trigger).toHaveFocus();
  });
  ...
  it("uses a Next-compatible link without mutating browser history", async () => {
    ...
    render(<SourceReader source={makeSource({ editionDate: "1991-12-11" })} onClose={onClose} />);
  ...
  it("does not expose transport errors to readers", async () => {
    ...
    expect(await screen.findByText("Unable to load this article right now.")).toBeInTheDocument();
