...
describe("Markdown renderer", () => {
  it("renders bold and italic", () => {
    const { container } = render(<Markdown>{"This is **bold** and *italic* text."}</Markdown>);
  ...
  it("maps agent [YYYY-MM-DD-N] citations via articleIdIndex", () => {
    ...
    render(<Markdown articleIdIndex={index}>{"X [1960-01-07-0] Y [1965-03-12-4] Z"}</Markdown>);
    const links = screen.getAllByRole("link");
    expect(links).toHaveLength(2);
    expect(links[0]).toHaveAttribute("href", "#ask-source-1");
    expect(links[1]).toHaveAttribute("href", "#ask-source-2");
  ...
  it("leaves agent citations unlinked when articleIdIndex has no match", () => {
    render(<Markdown>{"unmapped [1970-01-01-0] citation"}</Markdown>);
  ...
  it("external links open in a new tab", () => {
    render(<Markdown>{"See [the docs](https://example.com/docs) for more."}</Markdown>);
