...
function src(id: string, images: string[], captions: (string | null)[] = []): Source {
  return {
    id,
    ...
    distance: 0,
    imageUrls: images,
    imageCaptions: captions.length === images.length ? captions : images.map(() => null),
  };
}
...
describe("dedupSourceImages", () => {
  ...
  it("preserves nullable captions by parallel index", () => {
    const out = dedupSourceImages([src("a", ["u1", "u2"], [null, "second"])]);
...
describe("indexImagesByUrl", () => {
  it("keys by both raw and %20-encoded forms", () => {
    const images = dedupSourceImages([src("a", ["https://x/Page 1.webp"], ["cap"])]);
  ...
  it("keys by the fully decoded form as well", () => {
    const images = dedupSourceImages([src("a", ["https://x/Page%201.webp"], ["cap"])]);
    const ix = indexImagesByUrl(images);
    expect(ix.get("https://x/Page%201.webp")?.caption).toBe("cap");
    expect(ix.get("https://x/Page 1.webp")?.caption).toBe("cap");
  });
  ...
  it("tolerates malformed percent escapes without throwing", () => {
    const images = dedupSourceImages([src("a", ["https://x/bad%Zfile.webp"])]);
    expect(() => indexImagesByUrl(images)).not.toThrow();
  });

  it("returns the canonical index for each URL", () => {
    const images = dedupSourceImages([src("a", ["u1", "u2"]), src("b", ["u3"])]);
    const ix = indexImagesByUrl(images);
...
describe("extractInlineImageUrls", () => {
  it("returns an empty set when the markdown has no images", () => {
    const out = extractInlineImageUrls("A paragraph about [a source](https://example.com).");
  ...
  it("seeds both raw and decoded forms", () => {
    const out = extractInlineImageUrls("![cap](https://x/Page%201.webp)");
  ...
  it("tolerates a malformed percent escape", () => {
    ...
    expect(extractInlineImageUrls(bad).has("https://x/bad%Zfile.webp")).toBe(true);
  });
