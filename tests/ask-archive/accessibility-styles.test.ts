...
const styleFiles = [
  ...
  "composer.css",
  "typing-cursor.css",
].map((file) => read(`src/styles/components/ask-archive/${file}`));
...
describe("Ask accessibility style contracts", () => {
  const [landing, layout, caveat, transcript, sources, photos, markdown] = styleFiles;
  ...
  it("keeps every literal rem size in the reachable Ask surfaces at 12px or larger", () => {
    for (const css of styleFiles) {
      for (const match of css.matchAll(/font-size:\s*([\d.]+)rem/g)) {
        expect(Number(match[1]), `Sub-12px declaration: ${match[0]}`).toBeGreaterThanOrEqual(0.75);
      }
    }
  });
  ...
  it("maps compact labels, captions, badges, and stats to the 12px token", () => {
    ...
    for (const [css, selector, label] of contracts) {
      expect(rule(css, selector), label).toContain("font-size: var(--text-xs);");
    }
  });

  it("keeps disclosures and follow-up actions at least 44px tall", () => {
    expect(rule(sources, ".ask-source-toggle")).toContain("min-height: 44px;");
    expect(rule(transcript, ".ask-example-chip")).toContain("min-height: 44px;");
    // Both choices in the destructive confirmation must be equally
    // easy to hit — a cramped "Keep threads" is how people lose data.
    expect(rule(layout, ".ask-clear-dialog-button")).toContain("min-height: 44px;");
    expect(rule(landing, ".ask-landing-suggestion")).toContain("min-height: 84px;");
  });
  ...
  it("disables motion across the whole ask-* surface, not just the landing", () => {
    ...
    const reducedMotion = landing.slice(landing.indexOf("@media (prefers-reduced-motion: reduce)"));
