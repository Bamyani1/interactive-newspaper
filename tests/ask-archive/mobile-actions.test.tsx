...
describe("AskMobileActions", () => {
  ...
  it("wires disabled state from the can* props", () => {
    ...
    expect(screen.getByRole("button", { name: /start a new conversation/i })).toBeDisabled();
    expect(
      screen.getByRole("button", {
        name: /clear all threads/i,
      })
  ...
  it("exposes the action strip as a labeled group for AT users", () => {
    render(<AskMobileActions {...baseProps} />);
    expect(screen.getByRole("group", { name: /conversation actions/i })).toBeInTheDocument();
  });
});
