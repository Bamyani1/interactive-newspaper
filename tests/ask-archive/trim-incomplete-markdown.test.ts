...
describe("trimIncompleteMarkdown", () => {
  it("returns complete text unchanged", () => {
    const text = "In 1968 students marched [Source 1].\n\n## Aftermath\n\nThe **dean** responded.";
    expect(trimIncompleteMarkdown(text)).toBe(text);
  });

  it("holds back a partial citation bracket", () => {
    expect(trimIncompleteMarkdown("The dean resigned [Sour")).toBe("The dean resigned ");
  });
  ...
  it("holds back a partial image embed and its URL", () => {
    expect(trimIncompleteMarkdown("See the rally ![crowd](https://arch")).toBe("See the rally ");
    expect(trimIncompleteMarkdown("See the rally ![cro")).toBe("See the rally ");
  });
  ...
  it("strips an unclosed bold marker but keeps its words", () => {
    expect(trimIncompleteMarkdown("The **Board of Trustees voted")).toBe(
      "The Board of Trustees voted"
    );
  });

  it("leaves balanced bold alone", () => {
    expect(trimIncompleteMarkdown("The **Board** voted")).toBe("The **Board** voted");
  });
  ...
  it("holds trailing characters that may open a construct", () => {
    expect(trimIncompleteMarkdown("It was dramatic !")).toBe("It was dramatic ");
