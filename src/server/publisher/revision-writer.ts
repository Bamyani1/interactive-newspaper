...
function buildCandidates(input: WriteEditionRevisionInput): ContentCandidate[] {
  ...
  for (const article of transformArticles(edition)) {
    ...
    for (let pos = 0; pos < article.imageUrls.length; pos += 1) {
      ...
      const fileIndex = (raw?.image_files ?? []).findIndex((f) => basenameOf(f) === urlBasename);
      if (fileIndex >= 0) {
        const rawImage = raw?.images?.[fileIndex] as (OcrImage & { credit?: unknown }) | undefined;
...
export async function writeEditionRevision(
  ...
  const plannedAliases = candidates
    ...
    .filter(
      (
        alias
      ): alias is { legacyId: string; aliasKind: AliasKind; itemId: string; revisionId: string } =>
  ...
  return {
    ...
    counts: {
      ...
      pages: expectedPages,
      assets: [...manifestAssets.keys()].filter((hash) => !existingAssetHashes.has(hash)).length,
      refs: plannedRefs.filter((ref) => !existingRefKeys.has(`${ref.revisionId}\n${ref.position}`))
        .length,
    },
  };
}
