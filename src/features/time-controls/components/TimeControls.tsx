...
export const TimeControls: React.FC<TimeControlsProps> = ({ currentDate: routeDate }) => {
  ...
  const handleOptionKeyDown = (event: React.KeyboardEvent<HTMLButtonElement>, date: string) => {
  ...
  return (
    <header className="h-[var(--header-height)] w-full flex items-center justify-between px-6 text-[var(--color-text-header)] time-controls-header transition-colors duration-300 z-[var(--z-header)] fixed top-0 left-0">
