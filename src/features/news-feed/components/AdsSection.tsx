...
const TinyLiner: React.FC<TemplateProps> = ({ ad }) => (
  <div className="border-t border-b border-[var(--color-text-primary)]/30 bg-[var(--color-bg-secondary)]/60 p-2">
    <p className="font-typewriter text-xs leading-snug text-[var(--color-text-primary)]">
      <span className="font-semibold uppercase font-header tracking-wide">{ad.title}</span>
      <span className="text-[var(--color-accent-text)] mx-1">&mdash;</span>
      <span className="whitespace-pre-line">{adText(ad)}</span>
      {ad.phone && <span className="text-[var(--color-text-secondary)]"> {ad.phone}</span>}
    </p>
  </div>
);
...
const Bulletin: React.FC<TemplateProps> = ({ ad }) => (
  <div
    className="transform rotate-[-1deg] border border-[var(--color-text-primary)]/25 bg-[var(--color-bg-secondary)]/60 p-4 flex flex-col gap-2 relative"
...
const Marquee: React.FC<TemplateProps> = ({ ad }) => (
  <div className="border border-[var(--color-text-primary)]/25 border-t-2 border-t-[var(--stroke-accent-soft)] border-b-2 border-b-[var(--stroke-accent-soft)] bg-[var(--color-bg-secondary)]/60 p-4 flex flex-col text-center gap-2">
    <p
      className="font-header text-xs tracking-label-md text-[var(--color-accent-text)] opacity-50 select-none"
      aria-hidden="true"
    >
      &#9733; &#9733; &#9733; &#9733; &#9733;
    </p>
...
const Showcase: React.FC<TemplateProps> = ({ ad }) => (
  <div className="border border-[var(--color-border-default)] bg-[var(--color-bg-secondary)]/60 p-6 flex flex-col text-center gap-2 relative">
    <span
      className="absolute top-1 left-2 font-mono text-2xl leading-none text-[var(--color-rule-accent)] opacity-40 select-none"
      aria-hidden="true"
    >
      &#9556;
    </span>
    <span
      className="absolute top-1 right-2 font-mono text-2xl leading-none text-[var(--color-rule-accent)] opacity-40 select-none"
      aria-hidden="true"
    >
      &#9559;
    </span>
    <span
      className="absolute bottom-1 left-2 font-mono text-2xl leading-none text-[var(--color-rule-accent)] opacity-40 select-none"
      aria-hidden="true"
    >
      &#9562;
    </span>
    <span
      className="absolute bottom-1 right-2 font-mono text-2xl leading-none text-[var(--color-rule-accent)] opacity-40 select-none"
      aria-hidden="true"
    >
      &#9565;
    </span>
    ...
    <p
      className="font-mono text-xs text-[var(--color-rule-accent)] opacity-30 select-none"
      aria-hidden="true"
    >
      &#9552;&#9552;&#9552;&#9552;&#9552;&#9552;&#9552;&#9552;&#9552;&#9552;&#9552;&#9552;&#9552;&#9552;&#9552;&#9552;&#9552;&#9552;&#9552;&#9552;
    </p>
  ...
  if (displayAds.length === 0) {
    return (
      <section className="p-10 text-center border border-dashed rounded-sm opacity-70">
        <p className="font-header text-xl uppercase tracking-widest">No advertisements available</p>
      </section>
    );
  }
  ...
  return (
    <section className="w-full">
      <div className="flex items-center justify-between border-b pb-3 mb-6">
        <div>
          <p className="font-mono text-xs uppercase tracking-label-md opacity-70">Advertisements</p>
          <h3 className="font-header text-2xl md:text-3xl leading-tight">Display Ads</h3>
        </div>
      ...
      <div className="columns-1 sm:columns-2 gap-4 md:gap-5 mb-8">
        {visibleAds.map((ad, idx) => (
          <div key={`display-${ad.title}-${idx}`} className="break-inside-avoid mb-4 md:mb-5">
...
export const ClassifiedsSection: React.FC<ClassifiedsSectionProps> = ({ classifiedAds }) => {
  ...
  if (classifiedAds.length === 0) {
    return (
      <section className="p-10 text-center border border-dashed rounded-sm opacity-70">
        <p className="font-header text-xl uppercase tracking-widest">No classifieds available</p>
      </section>
    );
  }
  ...
  return (
    <section className="w-full">
      <div className="flex items-center justify-between border-b pb-3 mb-6">
        <div>
          <p className="font-mono text-xs uppercase tracking-label-md opacity-70">Classifieds</p>
          <h3 className="font-header text-2xl md:text-3xl leading-tight">Classified Listings</h3>
        </div>
        <span className="px-3 py-1 border border-[var(--color-text-primary)] text-xs uppercase tracking-widest font-semibold">
      ...
      <div className="columns-1 sm:columns-2 gap-4 md:gap-5 mb-8">
        {visibleAds.map((ad, idx) => (
          <div key={`classified-${ad.title}-${idx}`} className="break-inside-avoid mb-4 md:mb-5">
            <DisplayAd ad={ad} variant={variants[idx]} />
          </div>
        ))}
      </div>
