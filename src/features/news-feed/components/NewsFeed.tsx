...
export const NewsFeed: React.FC<NewsFeedProps> = ({
  ...
  const sortedEditions = useMemo(
    () => [...editions].sort((a, b) => a.localeCompare(b)),
    [editions]
  );
  ...
  const currentArticles = useMemo(
    () =>
      currentSection === "Top"
        ? []
        : (groupedArticles.find((g) => g.category === currentSection)?.articles ?? []),
  ...
  return (
    <div className="edition-feed-surface w-full bg-[var(--color-bg-primary)]">
      ...
      >
        ...
        <div className="flex flex-col max-w-5xl mx-auto px-4 md:px-6 w-full">
          {currentSection === "Top" ? (
            <TopStoriesPrintEdition heroArticle={heroArticle} featuredArticles={featuredArticles} />
          ) : currentSection === "Ads" ? (
            <AdsSection displayAds={displayAds} />
          ) : currentSection === "Classifieds" ? (
            <ClassifiedsSection classifiedAds={classifiedAds} />
          ) : currentArticles.length > 0 ? (
            <SectionPrintEdition key={currentSection} articles={currentArticles} />
          ) : (
            <div className="p-12 text-center opacity-60">
              <p className="font-body text-2xl italic mb-4">No stories found for this section.</p>
              <button
