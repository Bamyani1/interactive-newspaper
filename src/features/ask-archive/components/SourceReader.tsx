...
const paragraphsFrom = (text: string | null | undefined): string[] => {
  ...
  if (/<p[\s>]/i.test(text)) {
    ...
    const paras = ps.map((p) => p.textContent?.trim() ?? "").filter((p) => p.length > 0);
...
export const SourceReader: React.FC<SourceReaderProps> = ({ source, onClose }) => {
    ...
    fetch(`/api/editions/${source.editionDate}`)
      ...
      .then((data: { articles: EditionArticle[] }) => {
        if (!cancelled) {
          ...
          const match = data.articles.find((a) => a.id === source.id);
          setArticle(match ?? null);
          setError(match ? null : "Article not found in this edition.");
        }
      })
      ...
      >
        ...
        <>
          <div className="ask-reader-dateline">
            <span className="ask-reader-section">{source.category}</span>
            <span className="ask-reader-dot" aria-hidden="true">
              ·
            </span>
            <span className="ask-reader-date">{source.editionDate}</span>
            {article?.page ? (
              <>
                <span className="ask-reader-dot" aria-hidden="true">
                  ·
                </span>
                <span className="ask-reader-page">p. {article.page}</span>
              </>
            ) : null}
          </div>
          ...
          {source.byline ? <p className="ask-reader-byline">{source.byline}</p> : null}
          ...
          <div className="ask-reader-body" aria-busy={loading}>
            {loading ? (
              <p className="ask-reader-status" role="status" aria-live="polite">
                Loading…
              </p>
            ) : error ? (
              <p className="ask-reader-status" role="alert">
                {error}
              </p>
            ) : paragraphs.length > 0 ? (
              paragraphs.map((p, i) => <p key={i}>{p}</p>)
            ) : (
              <p className="ask-reader-status">No full text available for this article.</p>
            )}
          </div>
