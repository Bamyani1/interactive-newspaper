...
function saveCanvasAsPdf(
  ...
  pdf.addImage(image, "JPEG", PAGE_MARGIN_PT, y, printableWidth, imageHeight);
...
export async function exportConversationPdf(turns: TurnData[]): Promise<void> {
  ...
  try {
    ...
    const documentNode = host.querySelector<HTMLElement>(".ask-export-document");
    if (!documentNode) {
    ...
    const canvas = await html2canvas(documentNode, {
      ...
      onclone: (clonedDocument) => {
        ...
        const clonedExport = clonedDocument.querySelector<HTMLElement>(".ask-export-document");
        if (clonedExport) {
