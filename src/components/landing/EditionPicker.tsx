...
export function EditionPicker({
  ...
  const activateDecade = useCallback(
    (prefix: string) => {
  ...
  const handleTabKeyDown = (event: React.KeyboardEvent<HTMLButtonElement>, prefix: string) => {
  ...
  const handleOptionKeyDown = (
    ...
    if (event.key === "ArrowDown") nextIndex = (currentIndex + 1) % group.editions.length;
      ...
      nextIndex = (currentIndex - 1 + group.editions.length) % group.editions.length;
    else if (event.key === "Home") nextIndex = 0;
  ...
  return (
    ...
    >
          ...
          >
            ...
            >
              {group.editions.map((date) => {
                ...
                const isActiveOption = isActive && (activeDate ?? group.editions[0]) === date;
                return (
                  <button
                    key={date}
                    type="button"
                    ...
                    onKeyDown={(event) => handleOptionKeyDown(event, date, group)}
                  >
                    <span className="ep-date-item-date">
                      {formatEditionDate(date)}
                      {date === "1960-01-13" && " (golden)"}
                    </span>
                    <span className="ep-date-item-weekday">{formatWeekday(date)}</span>
                  </button>
                );
              })}
            </div>
      ...
      {/* Close */}
      <button type="button" className="ep-close-btn" onClick={() => closePicker()}>
        ✕ Close
      </button>
    </div>
  );
