...
export function EditionDateClient({
  ...
  const classifiedAds = useMemo(
    () =>
      ads.filter((a) =>
        a.adType ? a.adType === "classified" : a.body.length >= 80 && a.body.length < 200
      ),
    [ads]
  );
  ...
  const sections = useMemo(() => {
    ...
    if (classifiedAds.length > 0) {
      result.push({
        id: "Classifieds" as SectionId,
        label: "Classifieds",
        count: classifiedAds.length,
      });
    }
  ...
  useLayoutEffect(() => {
    ...
    feed.scrollTop = isExplicitNavigation ? 0 : (editionFeedScrollPositions.get(currentDate) ?? 0);
  ...
  return (
    <PageShell variant="default" hasHeader className="edition-background-shell">
      ...
      <MobileNav sections={sections} activeSection={activeSection} onSelect={handleSectionChange} />
    </PageShell>
  );
}
