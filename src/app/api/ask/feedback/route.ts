...
function isStringArrayOfShape(value: unknown, check: (item: unknown) => boolean): boolean {
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (isRagEvaluationMode()) {
    return NextResponse.json({ ok: true, persisted: false, evaluationMode: true }, { status: 202 });
  }
