...
export function computeRerankSignals(articles: RetrievedArticle[]): RerankSignals {
  ...
  const avgVectorDist =
    vectorArticles.length > 0
      ? vectorArticles.reduce((sum, a) => sum + (a.distance ?? 0), 0) / vectorArticles.length
      : null;
  ...
  const topThreeBothCount = articles.slice(0, 3).filter((a) => a.source === "both").length;
...
export function logRerankSignals(
  ...
  console.warn(
    JSON.stringify({
      ...
      avgVectorDist:
        signals.avgVectorDist !== null ? Number(signals.avgVectorDist.toFixed(4)) : null,
