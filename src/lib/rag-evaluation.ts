...
export function isRagEvaluationMode(env: NodeJS.ProcessEnv = process.env): boolean {
 ...
 */
export function getRagEvaluationConfig(env: NodeJS.ProcessEnv = process.env): RagEvaluationConfig {
  ...
  const spendCapUsd = rawCap ? Number(rawCap) : DEFAULT_EVALUATION_SPEND_CAP_USD;
  if (
