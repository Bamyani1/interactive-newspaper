...
function validateSlimArchive(archive) {
  ...
  assert(
    archive.tmax_c.length === EXPECTED_DAYS,
    `tmax_c length mismatch: ${archive.tmax_c.length}`
  );
  assert(
    archive.tmin_c.length === EXPECTED_DAYS,
    `tmin_c length mismatch: ${archive.tmin_c.length}`
  );
  assert(
    archive.is_estimated.length === EXPECTED_DAYS,
    `is_estimated length mismatch: ${archive.is_estimated.length}`
  );
