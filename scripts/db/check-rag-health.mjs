...
export function servingPlan(config) {
  ...
  return {
    ...
    params: [config.activeIndexBuildId, config.embeddingModel, config.textEmbeddingInputVersion],
  };
}
  ...
  if (!configuredStamp) {
    failures.push(
      `served.stamps: ${plan.table} holds no rows stamped ` +
        ...
        (stamps.length === 0
          ? "none"
          : stamps.map((s) => `(${s.embeddingModel}, ${s.embeddingInputVersion})`).join(", "))
    );
  }
  ...
  lines.push(
    report.served.stamps.length === 0
      ...
      : `     served.stamps           ${report.served.stamps
          .map((s) => `(${s.embeddingModel}, ${s.embeddingInputVersion}) x${s.rows}`)
          .join(", ")}`
  );
  lines.push(
...
async function main() {
  ...
  try {
    ...
    const { getRagRetrievalConfig } = indexConfigModule.default ?? indexConfigModule;
