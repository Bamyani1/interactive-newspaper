...
async function main() {
  ...
  const estimatedCost = (estimatedTextTokens / 1_000_000) * 0.2 + availableImages.length * 0.00012;
  ...
  for (const { image, loaded } of availableImages) {
    ...
    try {
      await retryOnce(`Image ${image.id}`, async () => {
        ...
        const inputHash = embeddingInputFingerprint(input, IMAGE_EMBEDDING_INPUT_VERSION);
