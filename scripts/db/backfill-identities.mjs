...
function chunkRevisionRows(rows) {
  ...
  for (const row of rows) {
    ...
    if (
      current.length > 0 &&
      (current.length >= REVISION_CHUNK_ROWS || currentChars + rowChars > REVISION_CHUNK_TEXT_CHARS)
    ) {
      chunks.push(current);
      current = [];
      currentChars = 0;
...
export async function backfillIdentities(executor) {
  ...
  for (const rows of chunkArray(issueInserts, INSERT_CHUNK_ROWS)) {
    statements.push(
      multiRowInsert("INSERT INTO issues (id, canonical_date)", rows, "ON CONFLICT (id) DO NOTHING")
    );
  }
...
async function main() {
  console.warn("NOTE: this backfill is idempotent and alias-preserving; it can be re-run safely.");
