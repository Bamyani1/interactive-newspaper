...
export async function exportBuildVectors(
  ...
  writeFileSync(path.join(dir, "rag_index_builds.json"), `${JSON.stringify(buildRow, null, 2)}\n`);
  ...
  const complete =
    onDisk.chunks === manifest.dbCounts.chunks && onDisk.images === manifest.dbCounts.images;
  if (complete) {
