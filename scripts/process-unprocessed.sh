...
# Batch orchestrator: discovers all editions in ocr/scans/unprocessed/
...
# Successfully processed editions are moved to ocr/scans/processed/.
# ────────────────────────────────────────────────────────────────

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
UNPROCESSED_DIR="$ROOT_DIR/ocr/scans/unprocessed"
...
if [[ "$PARALLEL" -eq 1 ]]; then
  ...
  echo "Logs: ocr/output/<date>/pipeline.log"
  echo ""
  ...
  process_one() {
    ...
    local date
    date=$(basename "$dir" | grep -oE '[0-9]{4}-[0-9]{2}-[0-9]{2}' | head -1)
    local log_dir="$ROOT_DIR/ocr/output/$date"
    mkdir -p "$log_dir"
    ...
    if bash "$SCRIPT" "$dir" > "$log_dir/pipeline.log" 2>&1; then
      ...
      echo "  ✗ $date FAILED (see ocr/output/$date/pipeline.log)"
    fi
  }
