...
export const PRIVATE_TABLES: ReadonlyArray<{ name: string; reason: string }> = Object.freeze([
  ...
  },
  {
    ...
    reason:
      "User conversation state (session ids, questions, answers); never read by this exporter.",
  },
  {
...
export async function exportTableRows(executor: QueryExecutor, table: string): Promise<Row[]> {
  ...
  for (;;) {
    const where = lastKey ? `WHERE ${pkTuple} > (${pk.map((_, i) => `$${i + 1}`).join(", ")})` : "";
...
export async function exportPublicCorpus(
  ...
  const manifest: CorpusManifest = {
    ...
    schemaProvenance: {
      source:
        "on-disk migration files (scripts/db/migrations); the schema_migrations ledger is never read",
...
async function main(): Promise<void> {
  ...
  if (!values.export && !values.import)
    fail("One of --export <dir> or --import <dir> is required.");
  ...
  console.log(
    JSON.stringify({ dir: path.resolve(values.import as string), inserted: counts }, null, 2)
  );
}
