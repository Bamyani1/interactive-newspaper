...
export function assertNotProductionUrl(evalUrl, prodUrl, options = {}) {
  ...
  if (!allowNonstandardName && !/eval/i.test(evalParts.host) && !/eval/i.test(evalParts.database)) {
    throw new Error(
...
function collectSchemaDiffs(expected, actual, at, out) {
  ...
  if (bothArrays) {
    if (expected.length !== actual.length) {
      out.push(`${at}: snapshot has ${expected.length} entries, live schema has ${actual.length}`);
      return;
    }
