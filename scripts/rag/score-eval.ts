...
function assertHoldoutScorable(options: ScoreRunOptions): void {
  ...
  if (!bands) {
    throw new Error("Holdout scoring requires locked acceptance bands (none provided).");
  }
  ...
  if (!candidateReceipt) {
    throw new Error("Holdout scoring requires a frozen candidate receipt (none provided).");
  }
...
export function scoreRun(options: ScoreRunOptions): ScoreReport {
  ...
  const evidenceById = new Map(evidence.questions.map((question) => [question.id, question]));
  ...
  for (const record of runFile.records) {
    ...
    if (question.expectedImagesAny?.length) {
      ...
      visualHits.push(question.expectedImagesAny.some((url) => attached.has(url)) ? 1 : 0);
    ...
    const citedRelevant = new Set(citations.filter((id) => relevant.has(id)));
    ...
    claimSupport.push(citations.length > 0 && citations.every((id) => relevant.has(id)) ? 1 : 0);
  }
  ...
  const metrics: Record<string, number | null> = {
    ...
    ndcgAt8: mean(ndcg8),
    evidenceGroupRecall: groupsTotal === 0 ? null : groupsSatisfied / groupsTotal,
    citationPrecision: citationsTotal === 0 ? null : citationsRelevant / citationsTotal,
    ...
    noAnswerCalibration: expectedRefusals === 0 ? null : correctRefusals / expectedRefusals,
    injectionResistance: injectionProbes === 0 ? null : injectionSafe / injectionProbes,
    ...
    fallbackRate: runFile.records.length === 0 ? null : fallbacks / runFile.records.length,
    errorRate: runFile.records.length === 0 ? null : errors / runFile.records.length,
    costPerQuestionUsd: runFile.records.length === 0 ? null : totalCost / runFile.records.length,
  };
 ...
 */
export function lockAcceptanceBands(options: LockAcceptanceBandsOptions): AcceptanceBandsFile {
  ...
  for (const [metric, margin] of Object.entries(margins)) {
    ...
    if (baseline === null || baseline === undefined) {
      throw new Error(`Cannot lock a band for ${metric}: baseline has no value for it.`);
    }
...
export function compareToBands(scores: ScoreReport, bands: AcceptanceBandsFile): BandComparison {
...
async function main(): Promise<void> {
  ...
  if (runFile.split === "holdout") {
    ...
    candidateReceipt = readJson<FreezeCandidateReceipt>(path.resolve(args.receipt));
