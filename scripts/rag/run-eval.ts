 ...
 */
export function orderQuestions(questions: EvalQuestionInput[]): EvalQuestionInput[] {
  const remaining = [...questions];
  const done = new Set<string>();
  const ordered: EvalQuestionInput[] = [];
  while (remaining.length > 0) {
    const index = remaining.findIndex((q) => !q.dependsOn || done.has(q.dependsOn));
    if (index === -1) {
      throw new Error(
...
function buildStages(body: AskResponseEnvelope): EvalRunStages {
  ...
  return {
    reformulation: {
      status: "ok",
      ms: null,
      summary: "reformulated query observable only; stage latency not in envelope",
      detail: {
    ...
    ftsRaw: unavailable("raw FTS candidate list is not exposed by the /api/ask envelope"),
    vectorRaw: unavailable("raw vector candidate list is not exposed by the /api/ask envelope"),
    fusion: {
      status: "ok",
    ...
    rerank: {
      status: "ok",
      ms: null,
      summary: "final post-rerank ranked order; rerank latency not in envelope",
      rankedSourceIds,
    },
...
export async function runEvalQuestions(options: RunEvalOptions): Promise<RunEvalResult> {
  ...
  for (const question of ordered) {
    const sessionId = question.dependsOn ? sessions.get(question.dependsOn) : undefined;
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      if (attempt < maxAttempts) {
        ...
        await sleep(Math.min(Number.isFinite(retryAfterSec) ? retryAfterSec : 60, 65) * 1000);
      }
    }
    ...
    const record: EvalRunRecord = {
      ...
      error: ok
        ...
        : {
            status,
            kind: body.kind ?? null,
            message: transportError ?? body.message ?? body.error ?? `HTTP ${status}`,
          },
    };
    records.push(record);
    capture?.(record);
...
export function loadBlindQuestions(filePath: string): {
  ...
  const questions: EvalQuestionInput[] = (raw.questions ?? []).map((entry) => {
    ...
    return {
      ...
      dependsOn: typeof entry.dependsOn === "string" ? entry.dependsOn : undefined,
...
async function main(): Promise<void> {
  ...
  if (!args.yes) {
    throw new Error("Refusing to run a live evaluation without --yes (spends real API budget).");
  }
  ...
  const driver: EvalDriver = {
    async ask(input) {
      const request = new serverModule.NextRequest("http://localhost:3000/api/ask", {
        method: "POST",
  ...
  writeFileSync(receiptPath, `${JSON.stringify(freezeCandidateOutputs(runFile), null, 2)}\n`);
