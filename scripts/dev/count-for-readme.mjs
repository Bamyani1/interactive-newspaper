...
async function main() {
  ...
  // Editions
  const edRows =
    await sql`SELECT COUNT(*)::int AS n, MIN(date) AS min_d, MAX(date) AS max_d FROM editions`;
  ...
  try {
    ...
    out.ask_session_turns = { error: String(e.message).slice(0, 80) };
  }
  ...
  try {
    ...
    out.ask_feedback = { error: String(e.message).slice(0, 80) };
  }
